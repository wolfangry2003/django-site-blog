...
class CreateComment(CanCreateMixin, CommentCreateMixin):
    ...
    def form_valid(self, form):
        ...
        if author_email:
            email = EmailMessage(
                "دیدگاه جدید",
                "دیدگاه جدیدی برای پست «{}» که شما نویسنده آن هستید، ارسال شده:\n{}{}".format(article, current_site,
                reverse('blog:article-detail', kwargs={'slug': article.slug})),
                to=[author_email]
            )
            email.send()
            ...
            if user_email:
                email = EmailMessage(
                    "دیدگاه دریافت شد",
                    "دیدگاه دریافت شد به زودی به آن پاسخ خواهیم داد.",
                    to=[user_email]
            )
            email.send()
            ...
            if parent_email:
                email = EmailMessage(
                    "پاسخ به دیدگاه شما",
                    "پاسخی برای دیدگاه شما در پست «{}» ثبت شده است، برای مشاهده آن بر روی لینک زیر کلیک کنید:\n{}{}".format(article, current_site,
                    reverse('blog:article-detail',kwargs={'slug': article.slug})),
                    to=[parent_email]
            )
            email.send()
